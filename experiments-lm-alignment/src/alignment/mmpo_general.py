...
class MMPOTrainer(RewardTrainer):
    ...
    def _full_dataset_e_step(self, train_dataloader) -> Tuple[List[Dict], List[torch.Tensor]]:
        ...
        Returns:
            epoch_batches: list of CPU-resident input dicts (one per batch)
            epoch_gammas:  list of CPU-resident gamma tensors  (one per batch)
        """
        self.model.eval()
    ...
    def train(self, resume_from_checkpoint=None, trial=None, ignore_keys_for_eval=None, **kwargs):  # noqa: D401
        ...
        for epoch in range(num_train_epochs):
            ...
            # ----------------------------------------------------------------
            self.model.train()
            for batch_idx, (cpu_inputs, gamma_cpu) in enumerate(zip(epoch_batches, epoch_gammas)):
                # Move batch and gamma back to GPU
                inputs = {
