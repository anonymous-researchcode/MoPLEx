...
def evaluate_posterior_head_ranking(
    ...
    """
    Evaluate the requested cluster metric flow on listwise test data:
      1) use observed ranking in each test sample to compute posterior over heads;
      2) choose argmax posterior head for that sample;
      3) use that head to rank candidates and compute ranking accuracy.
    """
    if listwise_dataset is None:
        return {}
    if script_args.loss_type != "maxmin_em" or script_args.num_heads is None or script_args.num_heads <= 1:
        return {}
    ...
    with torch.no_grad():
        for row in listwise_dataset:
            ...
            logits = model(input_ids=input_ids, attention_mask=attention_mask).logits  # (M, K) for multi-head
            if logits.ndim == 1:
            ...
            # Posterior over heads from observed ranking likelihood (Plackett-Luce log-likelihood).
            comp_logp = []
            for head_idx in range(num_heads):
                u = rewards[:, head_idx]
