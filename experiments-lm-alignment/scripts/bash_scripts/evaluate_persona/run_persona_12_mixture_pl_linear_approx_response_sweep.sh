...
CONFIG_PATH="${CONFIG_PATH:-recipes/qwen3-1b/dpo/persona/config_mixture_pl_top1_qlora.yaml}"
...
TRAIN_RESPONSE_COUNTS="${TRAIN_RESPONSE_COUNTS:-20 28}"
ANCHORS="${ANCHORS:-4}"
SEEDS="${SEEDS:-42 43}"
...
METRIC_FOR_BEST_MODEL="${METRIC_FOR_BEST_MODEL:-ranking_validation/mixture_posterior/top1_acc}"

# Linear approximation uses input-embedding gradients and should not be paired
# with gradient checkpoint recomputation in the current mixture LoRA path.
...
prepare_dataset_for_k() {
  ...
  bash "${SCRIPT_DIR}/generate_persona_subset.sh"
}
