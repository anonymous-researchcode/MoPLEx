...
loss_type="${LOSS_TYPE:-em_dpo}"
num_train_epochs="${NUM_TRAIN_EPOCHS:-5}"
max_steps="${MAX_STEPS:-2000}"
downsample_rate="${DOWNSAMPLE_RATE:-1.0}"
em_temperature="${EM_TEMPERATURE:-1.0}"
em_prior_smoothing="${EM_PRIOR_SMOOTHING:-1e-3}"
seeds="${SEEDS:-42 43}"
...
for seed in ${seeds}; do
  run_name="persona12_emdpo_cluster_${loss_type}_heads${num_heads}_steps${max_steps}_seed${seed}"
