...
DATASET_DIR="${DATASET_DIR:-${REPO_ROOT}/data/ultrafeedback_disagreement_if_truthfulness_allow_one_tie_train_augmented_Qwen3_0p6B_ds0p2_k6_s42}"
SEEDS="${SEEDS:-42}"
RANKING_SIZES="${RANKING_SIZES:-2}"
GENERATED_RANKING_SIZES="${GENERATED_RANKING_SIZES:-1 2}"
...
if [[ -z "${DIMENSIONS}" ]]; then
  if [[ -n "${BASE_STATS_PATH}" && -f "${BASE_STATS_PATH}" ]]; then
    DIMENSIONS="$(
...
DIMENSIONS="${DIMENSIONS:-instruction_following truthfulness}"
...
BASE_OUTPUT_DIR="${OUTPUT_DIR:-outputs/ultrafeedback-disagreement/mixture-pl-augmented-two-dim/qwen3-0.6b-${dataset_tag}}"
BASE_WANDB_NAME="${WANDB_NAME:-qwen3-0.6b-mixture-pl-augmented-${dataset_tag}}"
...
for generated_ranking_size in ${GENERATED_RANKING_SIZES}; do
  ...
  run_name="${BASE_WANDB_NAME}-m${ranking_size}-mp${generated_ranking_size}-ds${ratio_tag}-temp${temp_tag}-lr${lr_tag}-s${seed}"
  output_dir="${BASE_OUTPUT_DIR}/m${ranking_size}-mp${generated_ranking_size}-ds${ratio_tag}-temp${temp_tag}-lr${lr_tag}-s${seed}"

  echo "Launching augmented mixture PL: dataset=${DATASET_DIR}; dims=${DIMENSIONS}; m=${ranking_size}; m_prime=${generated_ranking_size}; seed=${seed}; downsample=${DOWNSAMPLE_RATIO}"
